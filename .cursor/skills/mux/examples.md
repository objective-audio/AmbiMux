...
出力（`workspace/output/`）:
- `video_ambimux.mov`（LPCM から APAC に変換された空間音声のみ、音声トラックは1本）
...
## Example 3: 外部オーディオなし → 埋め込み Ambisonics

入力（`workspace/mux-input/`）:
- `scene_b.mov`（外部オーディオなし、埋め込み 4ch LPCM あり）
- 同一 `.mov` に **4ch Ambisonics と 2ch ステレオ** が同居していても、出力は **Ambisonics を APAC にした1本の音声トラック** のみ
...
出力（`workspace/output/`）:
- `scene_b_ambimux.mov`（埋め込み Ambisonics を APAC に変換。検証ログ上は `Audio tracks: 1`）
...
3. `2026_0213_field.mov` → 外部オーディオなし → 埋め込み Ambisonics を確認 → `workspace/output/2026_0213_field_ambimux.mov`（埋め込み Ambisonics を APAC に変換、音声1トラック）
...
- 埋め込みの **Ambisonics** は 4・9・16 チャンネルである必要がある（モノ/ステレオのみのトラックは変換対象にならない）
