...
2. `ffprobe` で埋め込みを確認 → **先頭の音声トラックが 4/9/16ch（Ambisonics）**なら変換対象
3. `--video` のみで変換
...
- 埋め込みでは **先頭の音声トラック**が **Ambisonics（4・9・16 チャンネル）** である必要がある（モノ/ステレオは全トラックから検出され、別トラックがあれば任意で第2トラックに含まれる）
